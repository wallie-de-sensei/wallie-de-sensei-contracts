//! Security tests for `WallieDeSenseiFactory::init` and `set_stream_contract`.
//!
//! Covers:
...
//!   does not implement the `WallieDeSenseiStream` interface (EOA / non-contract /
...
fn deploy_factory(env: &Env) -> WallieDeSenseiFactoryClient<'_> {
    let factory_id = env.register_contract(None, WallieDeSenseiFactory);
    WallieDeSenseiFactoryClient::new(env, &factory_id)
}

fn deploy_stream(env: &Env) -> Address {
    env.register_contract(None, WallieDeSenseiStream)
}
...
/// `WallieDeSenseiStream` interface (here, the factory contract itself).
#[test]
fn test_init_rejects_non_fluxora_stream_contract() {
    ...
    let factory = deploy_factory(&env);
    let admin = Address::generate(&env);
    // A real deployed contract, but not a WallieDeSenseiStream — it has no version().
    let wrong_contract = env.register_contract(None, WallieDeSenseiFactory);
...
/// `WallieDeSenseiStream` deployment.
#[test]
...
/// when the new address is a valid `WallieDeSenseiStream` deployment: an
...
// ---------------------------------------------------------------------------
// version() smoke-check sanity: confirm it exercises a real WallieDeSenseiStream
// entrypoint and is unaffected by the stream contract's own init state.
// ---------------------------------------------------------------------------

/// The smoke check (`version()`) succeeds against a `WallieDeSenseiStream` contract
